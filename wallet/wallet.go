...
func NewWallet() *Wallet {
	...
	// Add version byte in front of RIPEMD-160 hasn (0x00 for Main Network).
	vd4 := make([]byte, 21)
	vd4[0] = 0x00
	copy(vd4[1:], digest3)
	...
	// Add the first 4 bytes from 7 at the end of extended RIPEMD-160 hash from 4 (25 bytes).
	dc8 := make([]byte, 25)
	copy(dc8[:21], vd4[:])
...
func (w *Wallet) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		PublicKey         string `json:"publuc_key"`
